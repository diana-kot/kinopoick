...
import { addToFav, removeFromFav } from '@/redux/features/favoriteSlice'
...
const FilmInfo = ({ film, isFav }: IFilmInfoProps) => {
	...
	const handleFavorite = (id: number) => {
		let searchTerm = id
		let itemId = favoriteFilms.find(i => i.id === searchTerm)
		itemId ? setFav(true) : setFav(false)
	}
	...
	return (
		<>
			<div
				...
				style={{
					backgroundImage: `url(${getImg(film.poster?.url)})`,
				}}></div>

			<div className={styles['film-info__wrapper']}>
				<div className={styles['film-info__btn']}>
					{fav ? (
						<Button
							text='Убрать из избранного'
							onClick={() => {
								dispatch(removeFromFav(film.id))
								setFav(!fav)
							}}
						/>
					) : (
				...
				{film.rating &&
					...
					film.rating.kp !== null && (
						<div className={styles['film-info__title-wrapper']}>
							<h1 className={styles['film-info__title']}>
								{film.name}
							</h1>
				...
				{getProfession().length > 0 && (
					<>
						{getProfession().map(profession => (
							<div
								key={profession}
								className={styles['film-info__about-item']}>
								<p className={styles['film-info__about-title']}>

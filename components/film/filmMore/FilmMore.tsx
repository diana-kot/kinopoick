...
const FilmMore = ({ film }: FilmMoreProps) => {
	return (
		<div className={styles['film-more']}>
			...
			{film.similarMovies && film.similarMovies.length > 0 && (
				<div className={styles['film-more__similar']}>
					...
					<ul className={styles['film-more__similar-list']}>
						{film.similarMovies.slice(0, 5).map(similarFilm => (
							<li
								key={similarFilm.id}
								className={styles['film-more__similar-item']}>
								<Link
									href={`/film/${similarFilm.id}`}
									className={
										styles['film-more__similar-link']
									}>
									<div
										className={
											styles['film-more__similar-img']
										}>
										<Image
											src={getImg(
												similarFilm.poster.previewUrl
											)}
										...
										{similarFilm.rating?.kp && (
											<div
												className={
													styles[
														'film-more__similar-rating'
													]
												}>
												{similarFilm.rating.kp.toFixed(
													1
												)}
											</div>
										)}
									...
									<p
										className={
											styles['film-more__similar-name']
										}>
										{similarFilm.name}
									</p>
									<p
										className={
											styles['film-more__similar-info']
										}>
			...
			{film.facts?.length > 0 && <FilmFacts film={film} />}
		</div>
	)
}

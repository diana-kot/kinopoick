...
const SearchItem = ({ film, index }: SearchItemProps) => {
	return (
		<Link href={`/film/${film.id}`} className={styles['search-item__link']}>
			...
			<div className={styles['search-item__info']}>
				<div className={styles['search-item__title-wrapper']}>
					...
					{film.top250 && (
						<span className={styles['search-item__top']}>
							топ-250
						</span>
					)}
				</div>
				<div className={styles['search-item__alt-title']}>
					<p className={styles['search-item__alt-name']}>
						...
						{film.year !== 0 && (
							<span className={styles['search-item__year']}>
								{film.year}
							</span>
						)}
					</p>
				...
				{film.description && (
					<p className={styles['search-item__desc']}>
						{film.description}
					</p>
				)}

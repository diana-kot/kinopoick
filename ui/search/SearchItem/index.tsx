...
export default function SearchItem({
	...
	const { addFilmsToHistory } = useFilmSearch()

	const handleClick = (film: ISingleSearchResult) => {
		setIsDropdownOpened(false)
		addFilmsToHistory(film)
	}
					...
					<div className={styles['search-item__bottom']}>
						<p className={styles['search-item__rating']}>
							...
							show.rating.kp === 0
								...
								: show.rating &&
								  show.rating.kp &&
								  show.rating.kp.toFixed(1)}
							,
						</p>
						<p className={styles['search-item__release-year']}>
							{show.year}
						</p>
					</div>
				</div>
